...
def detect4skal(hsv):
    ...
    # Noise Reduction using Opening process (Erosion next to Dilation) of mathematical morphology
    ...
    for i in range(len(contours)):
        ...
        if w*2<h  : # -- is "pepsi"-->show red border
            ...
            pepsiFn = 1
        else: # -- is not "pepsi"-->show green border
...
def makedir(numThread):
    for i in range(numThread):
        os.mkdir("new_%s"%i)

def eachdir(e,numPerDir,threads):
    print("Enter eachdir")
    #lists = [[] for z in range(threads)]

    for i in range (numPerDir):
        init = threads*i
        for a in range(threads):
            #lists[y+x].append(img)
            shutil.copyfile("supermarket/"+imgFile[a+init],"new_%s/"%a+imgFile[a+init])
