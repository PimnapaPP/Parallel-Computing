...
def main(x,numPerDir,imgFile):
    ...
    for i in range (numPerDir):
        img = cv2.imread("./supermarket/"+imgFile[(x*numPerDir)+i])
        ...
        if pepsi >= 2:
            shutil.copyfile("supermarket/"+imgFile[(x*numPerDir)+i],"RESULT/"+imgFile[(x*numPerDir)+i])
...
if __name__ == '__main__':
    ...
    w = [mp.Process(target=main,args=(x,numPerDir,imgFile,))for x in range(threads)]
    print(w)
    for j in w:
            j.start()
    for j in w:
